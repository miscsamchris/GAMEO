...
class EventRest(Resource):
    def get(self,name):
        ...
        if event!= None:
            return {"code":200,"id":event.id,"Event Name":event.event_name,"Event Date":event.event_date,"Event State":event.event_state,"Event Description":event.event_description,"Activities": [{"Activity ID":i.id, "Activity Name":i.activity_name} for i in list(event.activities)],"Users": [{"User email":i.user_email, "User Name":i.user_name} for i in list(event.users)]}
...
class ActivityRest(Resource):
    def get(self,name):
        activity=Activity.query.filter_by(activity_name=name).first()
        if activity!= None:
